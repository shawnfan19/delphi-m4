"""Frozen-history forecast: use the model's intensity at the last prompt position
as the per-horizon risk score. No sampling, no integration.

Valid when intensity is time-homogeneous between events (so ∫ λ dτ = λ · H with
H shared across participants → λ alone ranks the same as the integral). For a
time-inhomogeneous model, reintroduce a probe grid over [t0, t0 + H] and
integrate per horizon.
"""
...
args = TaskConfig.auto(ckpt="delphi-m4/new_blood/ckpt.pt")
...
for batch_idx in pbar:
    ...
    # last fused position has the largest age (prompt/biomarker tokens, sorted);
    # its logits are log-intensity from the prompt-end state. rank-equivalent to
    # λ · H for any horizon H, so serves as the predictor across all horizons.
...
for horizon in args.horizons:
    ...
    for gender_key, is_gender in {"female": is_female, "male": ~is_female}.items():
        ...
        n_ctl, n_case, auc = batched_mann_whitney_auc(predictor, ctl_mask, case_mask)
...
path = Path(DELPHI_CKPT_WRITE) / Path(args.ckpt).parent / "forecast_frozen.json"
