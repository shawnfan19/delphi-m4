...
def month_of_birth():
    ...
    if _mob_cache is None:
        ...
        mob = mob.withColumn(
            "year_month",
            F.to_date(
                F.concat_ws("-", F.col("year"), F.col("month_str"), F.lit("1")),
                "yyyy-MMMM-d",
            ),
        )
...
def assessment_age():
    ...
    if _assess_age_cache is None:
        ...
        for col in VISITS:
            ...
            diff_days = F.datediff(F.to_date(F.col(p53_col)), F.col("year_month")).cast(
                "double"
            )
...
def load_biomarker_df(fids: list[int | str], visits: list[str]):
    ...
    long_df = df.select(
        "eid", F.stack(F.lit(num_visits), *stack_args).alias(*alias_names)
    )
...
def build_spark_biomarker(
    ...
    histogram_rows = (
        visit_counts.groupBy("num_visits")
