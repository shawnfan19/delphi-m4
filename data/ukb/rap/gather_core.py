...
# %%
fields_to_get = (
    [eid_f, sex_f, year_f, month_f, assessment_f, bmi_f, smoking_f, alcohol_f]
    + cancer_codes["type"]
    + cancer_codes["date"]
    + fo_fields
)
...
# %%
df = df.withColumn("sex", F.when(F.col(sex_f.name) == 0, "female").otherwise("male"))

df = df.withColumn(
    "dob",
    F.make_date(
        F.col(year_f.name), F.col(month_f.name), F.lit(1)
    ),  # year, month, day=1
)

df = df.withColumn("doa", F.to_date(F.col(assessment_f.name)))
...
df_long = df_long.withColumn("age", F.datediff(F.col("date"), F.col("dob")))
...
# still ~200 entries would have negative age; not sure why but remove
...
df_filtered = df_long.join(
    F.broadcast(map_df), df_long.token == map_df.token, "left_semi"
)
df_final = df_filtered.join(
    F.broadcast(map_df), df_filtered.token == map_df.token, "left"
)
...
tokens.astype(np.uint32).tofile(os.path.join(odir, "data.bin"))
timesteps.astype(np.uint32).tofile(os.path.join(odir, "time.bin"))
...
p2i.to_csv(os.path.join(odir, "p2i.csv"))
